...
def p_array_ele_ref(p):
    """
    array_ele_ref : ref '[' expression ']'
    """
    p[0] = 'PUSHGP' + p[1] + 'PADD' + p[3] + 'LOADN\n'
    if global_debug:
        print('p_array_ele_ref')

def p_array_ele_ref_mat(p):
    """
    array_ele_ref : id_const '[' expression ']' '[' expression ']'
    """
    p[0] = 'PUSHGP' + p[1] + 'PADD' + p[3] + global_env[p[1]].dims[0] + 'MUL' + p[6] + 'ADD' + 'LOADN\n'
    if global_debug:
        print('p_array_ele_ref_mat')

# ------------------------------------------------------------ array
def p_array_subscript(p):
    """
    array_subscript : '[' expressionint_const ']'
    """
    p[0] = p[2]
    if global_debug:
        print('p_array_subscript')

def p_array_subscript_seq_1(p):
    """
    array_subscript_seq : array_subscript
    """
    p[0] = [p[1]]
    if global_debug:
        print('p_array_subscript_seq_1')

def p_array_subscript_seq_2(p):
    """
    array_subscript_seq : array_subscript_seq array_subscript
    """
    p[0] = p[1] + [p[2]]
    if global_debug:
        print('p_array_subscript_seq_2')
# ------------------------------------------------------------ assignment
def p_assignment(p):
    """
    assignment : ref assignment_operator expression
    """
